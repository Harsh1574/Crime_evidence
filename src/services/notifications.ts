...
function linkFor(userId: string, input: NotificationInput): string | null {
    if (input.evidenceId) return `/dashboard/${userId}/evidence/${input.evidenceId}`;
    if (input.caseId) return `/dashboard/${userId}/cases/${input.caseId}`;
    return null;
}

/** Notify a set of users (duplicates and empty IDs are ignored) */
export async function notifyUsers(userIds: (string | null | undefined)[], input: NotificationInput): Promise<void> {
...
export async function notifyPermissionHolders(permission: string, input: NotificationInput, excludeUserId?: string): Promise<void> {
