...
export function computeBlockHash(block: {
    ...
    return sha256(
        [block.blockNumber, block.prevHash, block.txId, block.assetId, block.function, block.cid ?? "", block.payloadHash].join("|")
    );
}

/** Append a transaction to the local ledger index (hash-linked to the previous block) */
async function appendBlock(entry: { txId?: string; assetId: string; fn: string; cid: string | null; payloadHash: string }): Promise<LedgerReceipt> {
...
// ---------------------------------------------------------------------------
let fabricContractPromise: Promise<any> | null = null;

async function getFabricContract(): Promise<any> {
...
export async function putAsset(write: LedgerWrite): Promise<LedgerReceipt> {
...
export async function deleteAsset(assetId: string): Promise<LedgerReceipt> {
    if (getLedgerMode() === "fabric") {
        const contract = await getFabricContract();
...
export async function readAsset(assetId: string): Promise<LedgerAsset | null> {
    ...
    if (!latest || latest.function === "DeleteAsset") return null;
    return {
        assetId,
        caseId: null,
        cid: latest.cid,
 ...
 */
export async function verifyAssetChain(assetId: string): Promise<{ valid: boolean; blocksChecked: number; brokenAt?: number }> {
