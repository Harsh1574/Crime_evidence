...
import { Actor, recordEvidenceChange, ServiceError, TERMINAL_STATUSES } from "./evidence.js";
...
export async function requestDisposal(evidenceId: string, reason: string, actor: Actor) {
    ...
    if (!reason?.trim()) throw new ServiceError(400, "reason is required");
    ...
    if (existing) throw new ServiceError(409, "There is already a pending disposal request for this evidence");
...
function normalizeDecision(decision: string): "approved" | "rejected" | null {
    const d = decision?.toLowerCase();
    if (d === "approved" || d === "approve") return "approved";
    if (d === "rejected" || d === "reject" || d === "denied" || d === "deny") return "rejected";
    return null;
}
...
export async function reviewDisposal(
    ...
    if (!decision) throw new ServiceError(400, "status must be approved or rejected");
    ...
    if (!request) throw new ServiceError(404, "Disposal request not found");
    if (request.status !== "pending") throw new ServiceError(409, `Request already ${request.status}`);
    ...
    await logActivity(
        actor,
        decision === "approved" ? "approved_disposal" : "rejected_disposal",
        "Evidence",
        request.evidenceId,
        request.evidence.evidenceNumber
    );
...
async function buildDisposalCertificate(evidenceId: string, reviewer: Actor, reason: string, note: string, when: Date): Promise<string> {
