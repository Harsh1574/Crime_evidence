...
import { Actor, ServiceError, verifyEvidence } from "./evidence.js";
...
export async function generateEvidenceReport(evidenceId: string, generatedBy: Actor): Promise<{ buffer: Buffer; fileName: string }> {
    ...
    if (ev.files.length === 0) doc.font(BODY).fontSize(9).text("No files attached.");
    ...
    if (ev.custodyEvents.length === 0) doc.font(BODY).fontSize(9).text("Evidence has not been transferred.");
    for (const e of ev.custodyEvents) {
        ...
        if (e.signature) field("  Signature", e.signature, true);
        doc.moveDown(0.2);
    }
    ...
    for (const v of ev.versions) {
        ...
        if (v.notes) doc.font(BODY).fontSize(9).text(`  ${v.notes}`);
