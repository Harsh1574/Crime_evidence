...
import { buildEvidenceSearchWhere, createEvidence, deleteEvidence, findEvidenceId, initiateTransfer, removeUploadedFiles, storeUploadedFiles, updateEvidence, } from "../services/evidence.js";
...
function toCreateInput(body, hasFile) {
    return {
...
async function idOr404(req, res) {
    const id = await findEvidenceId(req.params.id);
    if (!id)
        res.status(404).json({ error: `Evidence "${req.params.id}" not found (tried evidence ID, UUID and IPFS CID).` });
    return id;
}
// ---------------------------------------------------------------------------
// POST /api/evidence — create (JSON, or multipart with an optional file)
// ---------------------------------------------------------------------------
router.post("/", authenticate, requirePermission("register_evidence"), upload.any(), async (req, res) => {
...
// ---------------------------------------------------------------------------
router.post("/bulk", authenticate, requirePermission("register_evidence"), async (req, res) => {
    try {
        const list = req.body?.evidenceList;
        if (!Array.isArray(list) || list.length === 0) {
            res.status(400).json({ error: "evidenceList array is required" });
            return;
        }
        if (list.length > 50) {
            res.status(400).json({ error: "Maximum 50 items per request" });
            return;
        }
        const results = [];
...
// ---------------------------------------------------------------------------
router.get("/:id/chain-of-custody", authenticate, async (req, res) => {
    try {
        const id = await idOr404(req, res);
        if (!id)
            return;
...
// ---------------------------------------------------------------------------
router.post("/:id/chain-of-custody", authenticate, requirePermission("transfer_evidence"), async (req, res) => {
    try {
        const id = await idOr404(req, res);
        if (!id)
            return;
        const { toOfficer, toUserId, notes, reason } = req.body;
        const { custodyEvent, recipient, anchoring } = await initiateTransfer(id, toOfficer ?? toUserId, notes ?? reason, req.user);
        res.status(201).json({
            success: true,
...
//# sourceMappingURL=ledger-evidence.js.map