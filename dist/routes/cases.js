...
function canManageOfficers(user, caseRow) {
...
// POST /api/v1/cases
router.post("/", authenticate, requireAnyPermission("create_cases", "register_evidence"), async (req, res) => {
    try {
...
router.get("/:id", authenticate, async (req, res) => {
    try {
        const found = await findCase(req.params.id);
        if (!found) {
...
router.get("/:id/officers", authenticate, async (req, res) => {
    try {
        const found = await findCase(req.params.id);
        if (!found) {
            res.status(404).json({ error: "Case not found" });
...
router.post("/:id/officers", authenticate, requirePermission("manage_case_officers"), async (req, res) => {
    try {
        const found = await findCase(req.params.id);
        if (!found) {
            res.status(404).json({ error: "Case not found" });
        ...
        if (!canManageOfficers(req.user, found)) {
            res.status(403).json({ error: "Only the case creator or an admin can manage case officers." });
            return;
        }
        ...
        if (existing) {
            res.status(409).json({ error: `${officer.username} is already assigned to this case.` });
            return;
        }
...
router.delete("/:id/officers/:userId", authenticate, requirePermission("manage_case_officers"), async (req, res) => {
    try {
        const found = await findCase(req.params.id);
        if (!found) {
            res.status(404).json({ error: "Case not found" });
        ...
        const officer = await resolveUser(req.params.userId);
        const assignment = officer
            ? await prisma.caseOfficer.findUnique({ where: { caseId_userId: { caseId: found.id, userId: officer.id } } })
            : null;
        if (!officer || !assignment) {
            res.status(404).json({ error: "User is not assigned to this case." });
            return;
        }
...
router.post("/:id/boxes", authenticate, requirePermission("register_evidence"), async (req, res) => {
    try {
        ...
        await prisma.evidence.updateMany({ where: { caseId: updated.caseId }, data: { caseRefId: req.params.id } });
